...
export default async function FactoryBoard() {
  ...
  if (!factory?.board_eligible) return (
    <div className="max-w-3xl mx-auto px-4 py-8 text-center">
      <p className="text-[15px] font-bold" style={{ color: "#181818" }}>Board opens after your qualification run</p>
      <p className="text-[12px] mt-1" style={{ color: "#3E3A30" }}>Complete your first scored run and this page fills with open work.</p>
    </div>
  );
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-5">
      ...
      <p className="text-[12px] font-mono mb-3" style={{ color: "#3E3A30" }}>YOUR BID IS SEALED · AWARD NOTIFICATIONS COME FROM SESHSURE</p>
          ...
          return (
            <div key={p.id} className="px-4 py-3 border-b" style={{ borderColor: "#E7DFCE" }}>
              ...
              {myBid && !myBid.declined
                ? <p className="font-mono text-[12px] mt-2 font-bold" style={{ color: "#0D9488" }}>
                    ✓ YOUR BID: {(Number(myBid.price_per_cone_microcents) / 10000).toFixed(2)}¢/CONE — SEALED</p>
                : myBid?.declined
                ? <p className="font-mono text-[12px] mt-2" style={{ color: "#3E3A30" }}>DECLINED</p>
                : <BidForm postId={p.id} />}
            </div>
          );
        })}
        {!posts?.length && <p className="px-4 py-6 text-[13px]" style={{ color: "#3E3A30" }}>No open posts right now.</p>}
      </div>
    </div>
  );
}
