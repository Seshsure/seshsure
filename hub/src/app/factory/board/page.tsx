    ...
    <div className="max-w-2xl mx-auto px-4 py-5">
      ...
      <div className="rounded-xl border overflow-hidden" style={{ background: "#fff", borderColor: "#E4E1DA" }}>
        {(posts ?? []).map(p => {
          ...
          return (
            <div key={p.id} className="px-4 py-3 border-b" style={{ borderColor: "#E4E1DA" }}>
              <p className="text-[12px] font-bold" style={{ color: "#15181A" }}>
              ...
              {myBid && !myBid.declined
                ...
                : myBid?.declined
                ? <p className="font-mono text-[10px] mt-2" style={{ color: "#6E756B" }}>DECLINED</p>
                : <BidForm postId={p.id} />}
            </div>
          );
        })}
        {!posts?.length && <p className="px-4 py-6 text-[11px]" style={{ color: "#6E756B" }}>No open posts right now.</p>}
      </div>
    </div>
  );
}
