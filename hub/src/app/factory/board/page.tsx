...
export default async function FactoryBoard() {
  ...
  if (!factory?.board_eligible) return (
    <div className="max-w-3xl mx-auto px-4 py-8 text-center">
      ...
      <p className="text-[10px] mt-1" style={{ color: "#6E6A5E" }}>Complete your first scored run and this page fills with open work.</p>
    </div>
  );
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-5">
      ...
      <p className="text-[10px] font-mono mb-3" style={{ color: "#6E6A5E" }}>YOUR BID IS SEALED · AWARD NOTIFICATIONS COME FROM SESHSURE</p>
        ...
        {(posts ?? []).map(p => {
          ...
          return (
            <div key={p.id} className="px-4 py-3 border-b" style={{ borderColor: "#E7DFCE" }}>
              ...
              <p className="font-mono text-[8px] mt-0.5" style={{ color: "#6E6A5E" }}>
              ...
              {myBid && !myBid.declined
                ...
                : myBid?.declined
                ? <p className="font-mono text-[10px] mt-2" style={{ color: "#6E6A5E" }}>DECLINED</p>
                : <BidForm postId={p.id} />}
            </div>
          );
        })}
        {!posts?.length && <p className="px-4 py-6 text-[11px]" style={{ color: "#6E6A5E" }}>No open posts right now.</p>}
      </div>
    </div>
  );
}
