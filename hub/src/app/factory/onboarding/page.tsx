...
export default async function FactoryOnboarding() {
  ...
  const [{ data: sigs }, { data: acks }, { data: agreements }, { data: specs }, { data: fac }, { data: fdocs }] = await Promise.all([
    ...
    sb.from("factories").select("legal_name, wire_details_enc").eq("id", factoryId).single(),
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-6">
      ...
      <p className="text-[13px] mt-1" style={{ color: "#3E3A30" }}>Five steps: company, banking, documents, agreements, specifications. Rates and payment terms are confirmed with SeshSure directly.</p>
      <FactoryOnboardWizard
        factoryId={factoryId}
        termsDone={(sigs?.length ?? 0) > 0}
        specDone={(acks?.length ?? 0) > 0}
        docKeys={docKeys.length ? docKeys : ["factory_nda", "factory_non_circumvention", "factory_services"]}
        companyDone={!!fac?.legal_name}
        bankingDone={!!fac?.wire_details_enc}
        docsCount={fdocs?.length ?? 0}
