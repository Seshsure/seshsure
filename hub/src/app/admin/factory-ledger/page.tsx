  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pb-8">
      <div className="mt-4 flex items-center justify-between">
        ...
        <span className="font-mono text-[8px] px-2 py-1 rounded border" style={{ color: "#514C41", borderColor: "#E7DFCE" }}>XLSX EXPORT</span>
      </div>
          ...
          <thead>
            <tr style={{ background: "#FAF5EA" }}>
              {["#","FACTORY","COMPANY","QTY","RATE/CONE","FEES","TOTAL","KIND","FLAG"].map(h => (
                <th key={h} className="px-2 py-2 text-left border-b border-r" style={{ borderColor: "#E7DFCE", color: "#514C41", fontSize: 8 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {(lines ?? []).map((l, i) => (
              <tr key={l.id}>
                <td className="px-2 py-1.5 border-b border-r" style={{ borderColor: "#E7DFCE", color: "#7A7365" }}>{i + 1}</td>
                ...
                <td className="px-2 py-1.5 border-b border-r" style={{ borderColor: "#E7DFCE", color: "#514C41" }}>{l.kind.toUpperCase()}</td>
                <td className="px-2 py-1.5 border-b" style={{ borderColor: "#E7DFCE", color: "#C77800" }}>{l.discrepancy_flag ? `⚠ ${l.discrepancy_flag}` : ""}</td>
              </tr>
            ))}
            <tr style={{ background: "#FAF5EA" }}>
              <td colSpan={6} className="px-2 py-2 border-r text-right font-bold" style={{ borderColor: "#E7DFCE", color: "#514C41" }}>SUM</td>
