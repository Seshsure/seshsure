...
const GRADE_COLOR: Record<string, string> = { A: "#0D9488", B: "#8FD64B", C: "#C77800", D: "#F58B4B", F: "#E63946", "—": "#5C574A" };
    ...
    <div className="max-w-5xl mx-auto px-4 pb-8">
      ...
      <div className="mt-4 rounded-lg border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
        <div className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
          <span className="font-mono text-[12px] font-bold" style={{ color: "#3E3A30" }}>CLIENTS — GRADES COMPUTED FROM PAYMENT BEHAVIOR, NEVER TYPED</span>
        </div>
        ...
        {rows.map(({ c, g, basis, exposure, openOverdue }) => (
          <Link key={c.id} href={`/admin/clients/${c.id}`} className="flex items-center px-3 py-3 border-b" style={{ borderColor: "#E7DFCE" }}>
            <span className="w-8 h-8 rounded-lg flex items-center justify-center font-mono text-[16px] font-bold mr-3"
              style={{ background: `${GRADE_COLOR[g]}18`, color: GRADE_COLOR[g] }}>{g}</span>
            <div className="flex-1">
              <p className="text-[14px] font-semibold" style={{ color: "#181818" }}>
                {c.dba ?? c.legal_name}
                {c.hold_active && <span className="ml-2 font-mono text-[9px]" style={{ color: "#E63946" }}>🔒 HOLD</span>}
                {c.watch_flag && <span className="ml-1.5 font-mono text-[9px]" style={{ color: "#C77800" }}>👁</span>}
                {c.dormant && <span className="ml-1.5 font-mono text-[9px]" style={{ color: "#5C574A" }}>💤</span>}
              </p>
              <p className="font-mono text-[9px] mt-0.5" style={{ color: "#5C574A" }}>{basis}</p>
            </div>
            <div className="text-right">
              <p className="font-mono text-[13px] font-bold" style={{ color: openOverdue ? "#E63946" : "#181818" }}>{formatUSD(exposure)}</p>
              <p className="font-mono text-[9px]" style={{ color: "#5C574A" }}>EXPOSURE</p>
            </div>
          </Link>
        ))}
        {!rows.length && <p className="px-3 py-4 text-[13px]" style={{ color: "#5C574A" }}>No active clients yet.</p>}
      </div>
