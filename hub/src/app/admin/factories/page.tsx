    ...
    <div className="max-w-5xl mx-auto px-4 pb-8">
      {concentration !== null && (
        <div className="mt-4 p-3 rounded-lg border" style={{ background: "#FFFFFF", borderColor: concentration >= 80 ? "#E6394655" : "#E7DFCE" }}>
          ...
          <p className="font-mono text-[17px] font-bold" style={{ color: concentration >= 80 ? "#E63946" : "#181818" }}>
            {concentration}% <span className="text-[12px]" style={{ color: "#5C574A" }}>OF VOLUME ON TOP FACTORY</span>
          </p>
          {concentration >= 80 && <p className="font-mono text-[10px] mt-1" style={{ color: "#E63946" }}>DIVERSIFY VIA THE RUN BOARD — SECOND-SOURCE FILE RECOMMENDED</p>}
        </div>
      )}
      <div className="mt-4 rounded-lg border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
        <div className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
          <span className="text-[12px] font-mono font-bold" style={{ color: "#3E3A30" }}>FACTORIES — SCORES ARE COMPUTED, NEVER TYPED</span>
        </div>
        {(factories ?? []).map(f => {
            ...
            <Link key={f.id} href={`/admin/factories/${f.id}`} className="block px-3 py-3 border-b" style={{ borderColor: "#E7DFCE" }}>
              <div className="flex items-center justify-between">
                <p className="text-[12.5px] font-bold" style={{ color: "#181818" }}>
                  {f.name}
                  {f.flagship_approved && <span className="ml-2 font-mono text-[9px] px-1.5 py-0.5 rounded" style={{ background: "#0D948822", color: "#0D9488" }}>FLAGSHIP</span>}
                  {!f.board_eligible && <span className="ml-2 font-mono text-[9px] px-1.5 py-0.5 rounded" style={{ background: "#C7780022", color: "#C77800" }}>QUALIFYING</span>}
                </p>
                <span className="font-mono text-[10px]" style={{ color: "#5C574A" }}>{f.country ?? ""} · {f.payment_terms ?? "terms TBD"}</span>
              </div>
