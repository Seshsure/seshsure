...
const STAGE_COLOR: Record<string, string> = {
  lead: "#514C41", paperwork: "#C77800", sample: "#0D9488", onboarding: "#3B5BDB", won: "#0D9488", lost: "#7A7365",
};
    ...
    <div className="max-w-5xl mx-auto px-4 pb-8">
      ...
      <div className="grid grid-cols-6 gap-1.5 mt-4">
        {STAGES.map(s => (
          <div key={s} className="rounded-lg border p-2 text-center" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
            ...
            <p className="font-mono text-[6px] font-bold mt-0.5" style={{ color: "#7A7365" }}>{s.toUpperCase()}</p>
          </div>
        ))}
      </div>
      ...
      {bySource.size > 0 && (
        <div className="mt-3 rounded-lg border p-3" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
          <p className="font-mono text-[8px] font-bold mb-1.5" style={{ color: "#7A7365" }}>WHERE LEADS COME FROM</p>
          <div className="flex flex-wrap gap-1.5">
            {[...bySource.entries()].sort((a, b) => b[1] - a[1]).map(([src, n]) => (
              <span key={src} className="font-mono text-[8px] px-2 py-1 rounded" style={{ background: "#FAF5EA", color: "#514C41" }}>
                {src.toUpperCase()} · {n}
              </span>
            ))}
          </div>
        ...
        <div className="mt-3 rounded-lg border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#C7780044" }}>
          ...
          {(dormant ?? []).map(c => (
            <div key={c.id} className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
              ...
              <span className="font-mono text-[8px] ml-2" style={{ color: "#7A7365" }}>LAST ORDER {c.last_order_at?.slice(0, 10)}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="mt-3 rounded-lg border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
        <div className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
          <span className="font-mono text-[10px] font-bold" style={{ color: "#514C41" }}>ACTIVE PIPELINE</span>
        </div>
        ...
        {active.map(p => (
          <div key={p.id} className="px-3 py-2.5 border-b" style={{ borderColor: "#E7DFCE" }}>
            ...
            <p className="font-mono text-[8px] mt-0.5" style={{ color: "#7A7365" }}>
              {p.contact_name ?? ""}{p.email ? ` · ${p.email}` : ""} · VIA {p.lead_source.toUpperCase()}
            </p>
          </div>
        ))}
        {!active.length && <p className="px-3 py-4 text-[11px]" style={{ color: "#7A7365" }}>Pipeline empty — the /start intake link feeds this.</p>}
      </div>

      {/* recent samples */}
      <div className="mt-3 rounded-lg border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
        <div className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
          <span className="font-mono text-[10px] font-bold" style={{ color: "#514C41" }}>SAMPLES OUT — DAY 3 + DAY 10 FOLLOW-UPS AUTO-QUEUE</span>
        </div>
        {(samples ?? []).map(s => {
        ...
        {!samples?.length && <p className="px-3 py-4 text-[11px]" style={{ color: "#7A7365" }}>No samples in flight.</p>}
      </div>
    </div>
  );
}
