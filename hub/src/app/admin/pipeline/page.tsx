...
const STAGE_COLOR: Record<string, string> = {
  lead: "#6E6A5E", paperwork: "#C77800", sample: "#0D9488", onboarding: "#3B5BDB", won: "#0D9488", lost: "#9B9484",
};
    ...
    <div className="max-w-5xl mx-auto px-4 pb-8">
      ...
      <div className="grid grid-cols-6 gap-1.5 mt-4">
        {STAGES.map(s => (
          <div key={s} className="rounded-lg border p-2 text-center" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
            ...
            <p className="font-mono text-[6px] font-bold mt-0.5" style={{ color: "#9B9484" }}>{s.toUpperCase()}</p>
          </div>
        ))}
      </div>
      ...
      {bySource.size > 0 && (
        <div className="mt-3 rounded-lg border p-3" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
          <p className="font-mono text-[8px] font-bold mb-1.5" style={{ color: "#9B9484" }}>WHERE LEADS COME FROM</p>
          <div className="flex flex-wrap gap-1.5">
            {[...bySource.entries()].sort((a, b) => b[1] - a[1]).map(([src, n]) => (
              <span key={src} className="font-mono text-[8px] px-2 py-1 rounded" style={{ background: "#FAF5EA", color: "#6E6A5E" }}>
                {src.toUpperCase()} · {n}
              </span>
            ))}
          </div>
      ...
      {(dormant?.length ?? 0) > 0 && (
        <div className="mt-3 rounded-lg border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#C7780044" }}>
          <div className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
            <span className="font-mono text-[10px] font-bold" style={{ color: "#C77800" }}>DORMANT — WIN-BACK RUNNING</span>
          </div>
          {(dormant ?? []).map(c => (
            <div key={c.id} className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
              <span className="text-[11px]" style={{ color: "#181818" }}>{c.dba ?? c.legal_name}</span>
              <span className="font-mono text-[8px] ml-2" style={{ color: "#9B9484" }}>LAST ORDER {c.last_order_at?.slice(0, 10)}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="mt-3 rounded-lg border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
        <div className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
          <span className="font-mono text-[10px] font-bold" style={{ color: "#6E6A5E" }}>ACTIVE PIPELINE</span>
        </div>
          ...
          <div key={p.id} className="px-3 py-2.5 border-b" style={{ borderColor: "#E7DFCE" }}>
            <div className="flex items-center justify-between">
              <p className="text-[12px] font-semibold" style={{ color: "#181818" }}>
                {p.company}
                {p.show_capture && <span className="ml-2 font-mono text-[7px] px-1.5 py-0.5 rounded" style={{ background: "#3B5BDB22", color: "#3B5BDB" }}>{p.show_name ?? "SHOW"}</span>}
              </p>
              <span className="font-mono text-[8px] font-bold" style={{ color: STAGE_COLOR[p.stage] }}>{p.stage.toUpperCase()}</span>
            </div>
            <p className="font-mono text-[8px] mt-0.5" style={{ color: "#9B9484" }}>
              {p.contact_name ?? ""}{p.email ? ` · ${p.email}` : ""} · VIA {p.lead_source.toUpperCase()}
            </p>
          </div>
        ))}
        {!active.length && <p className="px-3 py-4 text-[11px]" style={{ color: "#9B9484" }}>Pipeline empty — the /start intake link feeds this.</p>}
      </div>

      {/* recent samples */}
      <div className="mt-3 rounded-lg border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#E7DFCE" }}>
        <div className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
          <span className="font-mono text-[10px] font-bold" style={{ color: "#6E6A5E" }}>SAMPLES OUT — DAY 3 + DAY 10 FOLLOW-UPS AUTO-QUEUE</span>
        </div>
        {(samples ?? []).map(s => {
          ...
          return (
            <div key={s.id} className="flex items-center px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
              <span className="flex-1 text-[11px]" style={{ color: "#181818" }}>
                ...
                {s.branded && <span className="ml-2 font-mono text-[7px]" style={{ color: "#0D9488" }}>BRANDED — HAND DELIVERY</span>}
              </span>
        ...
        {!samples?.length && <p className="px-3 py-4 text-[11px]" style={{ color: "#9B9484" }}>No samples in flight.</p>}
      </div>
    </div>
  );
}
