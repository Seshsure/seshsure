...
export default async function InvoiceDetail({ params }: { params: { id: string } }) {
  ...
  if (!inv) return <p className="p-8 text-sm" style={{ color: "#6E756B" }}>Invoice not found.</p>;
    ...
    <div className="max-w-2xl mx-auto px-4 py-5">
      <div className="rounded-xl border overflow-hidden" style={{ background: "#fff", borderColor: "#E4E1DA" }}>
        <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: "#E4E1DA" }}>
          <div>
            <p className="font-mono text-[13px] font-bold" style={{ color: "#15181A" }}>{inv.invoice_number}</p>
            <p className="font-mono text-[8px]" style={{ color: "#6E756B" }}>{inv.kind.toUpperCase()}{inv.due_date ? ` · DUE ${inv.due_date}` : ""}</p>
          </div>
          <p className="font-mono text-[16px] font-bold" style={{ color: "#15181A" }}>{formatUSD(remaining)}</p>
        </div>
        {(inv.invoice_line_items as { description: string; amount_cents: string }[] ?? []).map((l, i) => (
          <div key={i} className="flex justify-between px-4 py-2.5 border-b" style={{ borderColor: "#E4E1DA" }}>
            <span className="text-[11px]" style={{ color: "#15181A" }}>{l.description}</span>
            <span className="font-mono text-[11px]" style={{ color: "#15181A" }}>{formatUSD(BigInt(l.amount_cents))}</span>
          </div>
        ))}
      </div>
