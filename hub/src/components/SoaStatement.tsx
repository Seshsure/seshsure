...
export function SoaStatement({ lines, side }: { lines: SoaLine[]; side: "factory" | "owner" }) {
  ...
  let running = 0n;
  const withBalance = lines.map(l => { running += BigInt(l.total_cents); return { ...l, balance: running }; });
  const unconfirmed = lines.filter(l => !l.factory_confirmed_at && l.kind !== "opening_balance").length;
  const disputed = lines.filter(l => l.dispute_note).length;
