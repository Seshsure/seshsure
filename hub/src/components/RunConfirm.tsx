  ...
  return (
    <div className="flex gap-2 mt-2 items-end">
      <div className="flex-1">
        <label className="text-[8px] font-mono font-bold" style={{ color: "#6E756B" }}>PROMISE SHIP DATE</label>
        <input type="date" value={promise} onChange={e => setPromise(e.target.value)}
          className="w-full px-3 py-2 rounded-md border font-mono text-[11px]" style={{ borderColor: "#E4E1DA" }} />
      </div>
      <button onClick={confirm} disabled={state === "busy" || !promise}
        className="px-5 py-2 rounded-md font-bold text-[11px] disabled:opacity-50" style={{ background: "#15181A", color: "#fff" }}>
        Confirm run
      </button>
    </div>
  );
