  ...
  return (
    <div className="mt-3 rounded-lg border overflow-hidden" style={{ background: "#FFFFFF", borderColor: "#C7780044" }}>
      <div className="px-3 py-2 border-b" style={{ borderColor: "#E7DFCE" }}>
        <span className="font-mono text-[12px] font-bold" style={{ color: "#C77800" }}>DEMAND LETTERS — DRAFTED BY MACHINE, SENT BY YOU</span>
      </div>
      {letters.map(l => (
        <div key={l.id} className="border-b" style={{ borderColor: "#E7DFCE" }}>
          <button onClick={() => { setOpen(open === l.id ? null : l.id); setText(l.draftText); setState("idle"); }}
            ...
            <span className="font-mono text-[10px] font-bold" style={{ color: l.status === "draft" ? "#C77800" : l.status === "approved_sent" ? "#0D9488" : "#5C574A" }}>
          ...
          {open === l.id && l.status === "draft" && (
            <div className="px-3 pb-3">
              <textarea value={text} onChange={e => setText(e.target.value)} rows={14}
                className="w-full px-3 py-2 rounded font-mono text-[11px] leading-relaxed border outline-none"
                style={{ background: "#FAF5EA", borderColor: "#E7DFCE", color: "#181818" }} />
              <div className="flex gap-1.5 mt-2">
                {(["email","certified_mail","both"] as const).map(v => (
                  <button key={v} onClick={() => setVia(v)} className="flex-1 py-1.5 rounded font-mono text-[10px] font-bold border"
                    style={{ background: via === v ? "#181818" : "transparent", color: via === v ? "#FAF5EA" : "#3E3A30", borderColor: "#E7DFCE" }}>
                    {v.replace("_", " ").toUpperCase()}
                  </button>
                ))}
              </div>
              {state === "done" ? <p className="text-[13px] font-bold mt-2 text-center" style={{ color: "#0D9488" }}>✓ Done</p> : (
                <div className="flex gap-2 mt-2">
                  <button onClick={() => act(l.id, "withdraw")} disabled={state === "busy"}
                    className="flex-1 py-2.5 rounded font-bold text-[13px] border" style={{ borderColor: "#E7DFCE", color: "#3E3A30" }}>Withdraw</button>
                  <button onClick={() => act(l.id, "approve_send")} disabled={state === "busy"}
                    className="flex-[2] py-2.5 rounded font-bold text-[13px]" style={{ background: "#C77800", color: "#FAF5EA" }}>
