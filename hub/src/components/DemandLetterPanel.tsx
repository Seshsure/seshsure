  ...
  return (
    <div className="mt-3 rounded-lg border overflow-hidden" style={{ background: "#14181B", borderColor: "#F5B84B44" }}>
      <div className="px-3 py-2 border-b" style={{ borderColor: "#262C31" }}>
        <span className="font-mono text-[10px] font-bold" style={{ color: "#F5B84B" }}>DEMAND LETTERS — DRAFTED BY MACHINE, SENT BY YOU</span>
      </div>
      {letters.map(l => (
        <div key={l.id} className="border-b" style={{ borderColor: "#262C31" }}>
          <button onClick={() => { setOpen(open === l.id ? null : l.id); setText(l.draftText); setState("idle"); }}
            ...
            <span className="font-mono text-[8px] font-bold" style={{ color: l.status === "draft" ? "#F5B84B" : l.status === "approved_sent" ? "#2DD4BF" : "#5C666D" }}>
              ...
              <div className="flex gap-1.5 mt-2">
                {(["email","certified_mail","both"] as const).map(v => (
                  <button key={v} onClick={() => setVia(v)} className="flex-1 py-1.5 rounded font-mono text-[8px] font-bold border"
                    style={{ background: via === v ? "#E8EAEC" : "transparent", color: via === v ? "#0C0F11" : "#8B949C", borderColor: "#262C31" }}>
                    {v.replace("_", " ").toUpperCase()}
                  </button>
                ))}
              </div>
              {state === "done" ? <p className="text-[11px] font-bold mt-2 text-center" style={{ color: "#2DD4BF" }}>✓ Done</p> : (
                <div className="flex gap-2 mt-2">
                  <button onClick={() => act(l.id, "withdraw")} disabled={state === "busy"}
                    className="flex-1 py-2.5 rounded font-bold text-[11px] border" style={{ borderColor: "#262C31", color: "#8B949C" }}>Withdraw</button>
                  <button onClick={() => act(l.id, "approve_send")} disabled={state === "busy"}
                    className="flex-[2] py-2.5 rounded font-bold text-[11px]" style={{ background: "#F5B84B", color: "#0C0F11" }}>
