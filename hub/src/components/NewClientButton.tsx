...
export function NewClientButton() {
  ...
  const inp = "w-full mt-1 px-3 py-2 rounded-lg text-[14px] border-2 outline-none";
  const lbl = "eyebrow block mt-3";

  return (
    <>
      <button onClick={() => setOpen(true)} className="punch-sm px-4 py-2 rounded-lg font-bold text-[13px]"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "#18181866" }} onClick={() => setOpen(false)}>
          <div className="w-full max-w-md rounded-xl punch p-5" style={{ background: "#FAF5EA" }} onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setF({ ...f, invite: !f.invite })} className="flex items-center gap-2 mt-4">
              <span className="text-[16px]">{f.invite ? "☑" : "☐"}</span>
              <span className="text-[13px] font-bold" style={{ color: "#181818" }}>Send portal invite now (needs email)</span>
            </button>
            <button onClick={submit} disabled={state === "busy" || f.legalName.length < 2 || f.leadSource.length < 2}
              className="punch-sm w-full mt-4 py-2.5 rounded-lg font-bold text-[14px] disabled:opacity-50" style={{ background: "#0D9488", color: "#fff" }}>
