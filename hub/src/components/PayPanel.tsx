...
export function PayPanel({ invoiceId, remainingCents }: { invoiceId: string; remainingCents: string }) {
  ...
  if (state === "done") return (
    <div className="mt-4 rounded-xl border p-5 text-center" style={{ background: "#fff", borderColor: "#E4E1DA" }}>
      ...
      <p className="text-[10px] mt-1 font-mono" style={{ color: "#6E756B" }}>ACH DEBITS IN THE NEXT DAILY BATCH · RECEIPT ON CLEARANCE</p>
    </div>
  );

  return (
    <div className="mt-4 rounded-xl border p-4" style={{ background: "#fff", borderColor: "#E4E1DA" }}>
      <label className="font-mono text-[9px] font-bold" style={{ color: "#6E756B" }}>PAY AMOUNT (USD)</label>
      <input value={amount} onChange={e => setAmount(e.target.value.replace(/[^\d.]/g, ""))}
        inputMode="decimal" className="w-full mt-1 px-3 py-3 rounded-lg border font-mono text-[18px]"
        style={{ borderColor: "#E4E1DA", color: "#15181A" }} />
      ...
      <p className="font-mono text-[8px] mt-3 text-center" style={{ color: "#9B9F98" }}>
