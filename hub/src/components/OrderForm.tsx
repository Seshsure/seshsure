...
export function OrderForm({ products, addresses }: { products: P[]; addresses: A[] }) {
  ...
  if (state === "done") return (
    <div className="rounded-xl border p-5 text-center" style={{ background: "#fff", borderColor: "#E7DFCE" }}>
      ...
      <p className="text-[11px] mt-2" style={{ color: "#6E6A5E" }}>{msg}</p>
  ...
  return (
    <div className="rounded-xl border p-4" style={{ background: "#fff", borderColor: "#E7DFCE" }}>
      {items.map((it, i) => (
        <div key={i} className="flex gap-2 items-end mb-2">
          <div className="flex-[2]">
            <label className={lbl} style={{ color: "#6E6A5E" }}>PRODUCT</label>
          ...
          <div className="flex-1">
            <label className={lbl} style={{ color: "#6E6A5E" }}>CONES</label>
          ...
          {items.length > 1 && <button onClick={() => setItems(items.filter((_, n) => n !== i))} className="pb-3 text-[10px] font-mono" style={{ color: "#6E6A5E" }}>✕</button>}
        </div>
      ))}
      ...
      <label className={lbl} style={{ color: "#6E6A5E" }}>SHIP TO</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={lbl} style={{ color: "#6E6A5E" }}>YOUR PO NUMBER *</label>
          <input value={po} onChange={e => setPo(e.target.value)} className={inp} style={{ borderColor: "#E7DFCE" }} />
        </div>
        <div>
          <label className={lbl} style={{ color: "#6E6A5E" }}>WEEKS THIS ORDER LASTS YOU</label>
      ...
      <label className={lbl} style={{ color: "#6E6A5E" }}>SPECIAL INSTRUCTIONS</label>
      ...
      <p className="text-[8px] font-mono mt-3 text-center" style={{ color: "#6E6A5E" }}>
