...
export function OrderForm({ products, addresses }: { products: P[]; addresses: A[] }) {
  ...
  if (state === "done") return (
    <div className="rounded-xl border p-5 text-center" style={{ background: "#fff", borderColor: "#E7DFCE" }}>
      ...
      <p className="text-[13px] mt-2" style={{ color: "#3E3A30" }}>{msg}</p>
      <a href="/portal" className="inline-block mt-4 px-5 py-2.5 rounded-lg font-bold text-[14px]" style={{ background: "#181818", color: "#fff" }}>Back home</a>
    </div>
  );
  ...
  return (
    <div className="rounded-xl border p-4" style={{ background: "#fff", borderColor: "#E7DFCE" }}>
      {items.map((it, i) => (
        <div key={i} className="flex gap-2 items-end mb-2">
          <div className="flex-[2]">
            <label className={lbl} style={{ color: "#3E3A30" }}>PRODUCT</label>
          ...
          <div className="flex-1">
            <label className={lbl} style={{ color: "#3E3A30" }}>CONES</label>
          ...
          {items.length > 1 && <button onClick={() => setItems(items.filter((_, n) => n !== i))} className="pb-3 text-[12px] font-mono" style={{ color: "#3E3A30" }}>✕</button>}
        </div>
      ))}
      <button onClick={() => setItems([...items, { productId: products[0]?.id ?? "", quantity: "" }])}
        className="w-full py-2 rounded-lg text-[13px] font-bold border mb-1" style={{ borderColor: "#0D9488", color: "#0D9488" }}>+ Add product</button>

      <label className={lbl} style={{ color: "#3E3A30" }}>SHIP TO</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={lbl} style={{ color: "#3E3A30" }}>YOUR PO NUMBER *</label>
          <input value={po} onChange={e => setPo(e.target.value)} className={inp} style={{ borderColor: "#E7DFCE" }} />
        </div>
        <div>
          <label className={lbl} style={{ color: "#3E3A30" }}>WEEKS THIS ORDER LASTS YOU</label>
      ...
      {artId && <p className="font-mono text-[10px] mt-1" style={{ color: "#0D9488" }}>✓ ART ATTACHED TO THIS ORDER</p>}

      <label className={lbl} style={{ color: "#3E3A30" }}>SPECIAL INSTRUCTIONS</label>
      ...
      {state === "err" && <p className="text-[12px] font-mono mt-2" style={{ color: "#D62839" }}>{msg}</p>}
      <p className="text-[10px] font-mono mt-3 text-center" style={{ color: "#3E3A30" }}>
