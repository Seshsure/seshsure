...
- ✅ AI read-and-draft panels (owner-only): /api/ai/draft gathers context SERVER-SIDE from the real record, drafts via Anthropic API in per-task voice constitutions (dispute client reply — never promises pre-ruling, never names suppliers; factory note — warm/numbered-questions/non-accusatory; collections — firm-human, escalates with age; supplier WhatsApp style). Editable draft + copy; NOTHING auto-sends; every draft logged. Gated on ANTHROPIC_API_KEY (graceful 503 until set — add to accounts list).
